...
2. Implement `HookTamperDetector` using cryptographic SHA-256 signatures to detect unauthorized modifications to `.git/hooks/`. As of Phase 61, `HookTamperDetector` is a thin compatibility view over `TypedArtifactStore` (`.rush/memory.db`), the canonical store: signatures are read from the live `.rush/hook_signatures.json` when present (regenerated by `rush hook install`), falling back to the store once the file has been migrated (renamed `.migrated`).
