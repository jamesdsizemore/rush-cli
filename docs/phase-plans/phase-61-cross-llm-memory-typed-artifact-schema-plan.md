...
- **Security Boundary:**
  ...
  4. Promoted (`STATED`) records carry a SHA-256 integrity checksum (`hashlib.sha256(content_bytes).hexdigest()`, `HookTamperDetector`'s hashing primitive only — not its file-separation guarantee) computed at promotion time and re-verified at recall; a mismatch is fail-closed and raises, never silently ignored. **Correction: this is a corruption-detection checksum, not a tamper-evident signature.** `HookTamperDetector` derives its guarantee from storing the hash in a file (`.rush/hook_signatures.json`) physically separate from the hashed hook scripts (`.git/hooks/*`) — a writer must touch two locations to forge both consistently. This phase's `signature` column lives in the *same* `memory_artifacts` row as the `content` column it hashes (§6.1); any writer with `UPDATE` access to that row — including a bug in this phase's own `write()`/promotion code — can recompute `hashlib.sha256(new_content).hexdigest()` and write both columns in the same statement, defeating detection entirely. No secret key or external authority is involved (plain SHA-256, no HMAC), so this catches accidental corruption (partial write, bit rot, a hand-edited row) — it does not catch a deliberate content edit by anything with DB write access. Do not represent this as tamper evidence against a malicious database writer in `docs/adr/0049-*.md` or any synced doc (§8.2).
...
3. **Drift 3 (corrected): a SQLite WAL precedent already exists — `src/rush/cache.py`'s `ResultCache`.** `journal_mode` appears zero times across the five *memory-subsystem* files checked (`failure_ledger.py`, `token_economy/ccr_store.py`, `patch/memory.py`, `hook/tamper_detector.py`, `memory/transactions.py`), but `ResultCache` (`cache.py:91-127`, docstring: "Thread-safe SQLite result cache with WAL mode, LRU eviction, and self-healing") already runs `PRAGMA journal_mode=WAL;`, `PRAGMA synchronous=NORMAL;`, and a `PRAGMA integrity_check;` self-heal-on-failure sequence in `_init_db()`, over a connection opened with `timeout=10.0` (`cache.py:103`) — this phase's `TypedArtifactStore` is not the first WAL-mode SQLite connection in the codebase. `TypedArtifactStore._init_db()` (§6.1) should reuse `ResultCache._init_db()`'s initialization and failure-policy pattern (WAL + `synchronous=NORMAL` + integrity-check-with-self-heal + a connection timeout) rather than inventing a first-of-kind store from nothing; it need not, and should not, reuse `ResultCache`'s table schema, since the two stores hold unrelated data (`.rush/cache.db` stays `ResultCache`'s and `PatchMemoryStore`'s file per §5's table and P61.3.2 action 3 — `TypedArtifactStore` uses its own `.rush/memory.db`, §6.1).
...
6. **Drift 6: `src/rush/continuity/providers.py`'s `provider_command()` dict is not cross-LLM memory transport — it is bounded, permission-gated session-resume CLI dispatch.** `provider_command(provider, handoff)` (`providers.py:35-79`) maps `"claude_code"`/`"codex_cli"`/`"antigravity_cli"` to a CLI binary + argv that launches an external coding CLI to *continue the same task* from a rendered handoff prompt; `resume_provider()` (`providers.py:394-485`) gates it behind `ExecutionPermissions(network=True)` via `_validate_provider_access()`, spawns it as a bounded `subprocess.run(..., shell=False, timeout=120)`, and is frozen by `tests/test_providers.py::test_continuity_provider_resume_uses_verified_cli_contracts` (L561-574) and `::test_direct_cli_resume_has_permission_gated_bounded_process_behavior` (L579-629, asserting `denied["status"] == "skipped"`, `kwargs["shell"] is False`, `kwargs["timeout"] == 120`). This is a different concern from P61.10's cross-tool memory-handoff transport (writing/reading `MemoryArtifact` rows for another tool to later recall) and must not be conflated with it. P61.10 builds `src/rush/memory/transport.py` as wholly new code; it does not replace, wrap, or redirect `provider_command()`'s call sites, and `src/rush/continuity/providers.py` is never a P61.10 Allowed Write.
...
| Cross-tool transport | native SDK / ACP / dedicated-file tiers (verified real: `claude-agent-sdk`, `codex app-server`, `agent-client-protocol`) | P61.10 | none — new code. `src/rush/continuity/providers.py`'s `provider_command()` routes dict is bounded, permission-gated session-**resume** CLI dispatch (§2.1 Drift 6), not memory-handoff transport; P61.10 must not replace or touch its call sites. |
...
class MemoryArtifact:
    ...
    origin_kind: str | None = None # migration provenance tag, e.g. "flight_event"/"hook_signature"/"checkpoint"/"failure_ledger"/"patch_memory"/"preference"/"invariant"/"session_memory"; None for rows written directly by MemoryTool, never by migration.py
    origin_id: str | None = None   # migration-source-derived natural key (flight event = sha256(session_id + timestamp + event_type); checkpoint = its name; failure_ledger = its fingerprint). None for non-migrated rows. `(origin_kind, origin_id)` is the idempotency key T-61.22 checks against — `id` (a fresh uuid4 per write) cannot serve this purpose, since a retried migration would generate a new uuid4 and pass an `id`-based existence check every time.
```
...
3. **Integrity-checksum invariant (renamed from "Signature invariant" — corrected, see §1 Security Boundary item 4):** `promoted_at is not None` implies `signature is not None`; a record cannot be `STATED` without a checksum, and `recall()` must re-verify `hashlib.sha256(content_bytes).hexdigest() == signature` for every `STATED` row it returns, raising (not silently ignoring) on mismatch. This check is unkeyed SHA-256 over the same row's `content`, stored in the same row — it detects accidental corruption only, not a deliberate edit by anything with `UPDATE` access to `memory_artifacts` (that actor can recompute a matching digest in the same statement). It is not, and must not be documented as, cryptographic tamper evidence against a database-level attacker; `HookTamperDetector`'s stronger guarantee comes from storing signature and hashed content in two physically separate files, a property this single-row schema does not reproduce.
...
8. **Recall-is-the-only-defended-read invariant:** `recall(subject: MemorySubject, query: str) -> list[MemoryArtifact]` is the only method that may hand `content` to a caller: it runs the identical FTS5 `memory_fts` match `search()` uses (same ranking, same `subject` scoping), then applies Invariants 3 (signature re-verify), 4 (Trojan-source scan), and 6 (staleness check) to each matched row before returning it — a raise from any of those checks propagates out of `recall()`. `search(subject: MemorySubject, query: str) -> list[MemoryArtifact]` (P61.7.2) exists only to let a caller test for a match's existence/rank without paying `recall()`'s per-row defense cost; any caller that consumes `content` must call `recall()`, never `search()`. Phase 62's cache gate (`docs/phase-plans/phase-62-memory-integration-layer-plan.md` §6.1) is this codebase's first caller for which the distinction is load-bearing, not stylistic.
9. **Failure/decision-recall query invariant:** `recall(subject: MemorySubject, query: str)` accepts `subject="failure"` and `subject="architectural_decision"` exactly as it accepts `subject="domain_knowledge"` — no per-subject variant method exists; a caller matching against either subject uses the identical `recall()` signature Invariant 8 defines.

---

## 7. Contract Test Inventory (T-61.01 through T-61.39)
...
| T-61.04 | `tests/test_phase61_store.py` | `test_recall_rescans_stated_signature_and_raises_on_mismatch` | Two assertions in one test, both required (a single implementation that only checks the stored `signature` field's presence/format, without recomputing `hashlib.sha256(content_bytes).hexdigest()` against the row's actual current `content`, must fail one of them): (a) promotes a record, corrupts its stored `signature` field directly (content unchanged), asserts `recall()` raises; (b) promotes a second record, leaves `signature` untouched but mutates the row's `content` directly (bypassing `write()`), asserts `recall()` raises on the now-mismatched recomputed hash — proving the check is a real `hashlib.sha256(content_bytes).hexdigest() == signature` recomputation against current content, not merely a check that some `signature` value is present or well-formed. |
...
| T-61.37 | `tests/test_phase61_migration.py` | `test_migration_rerun_after_partial_completion_does_not_duplicate_or_drop_rows` | Runs migration once, then deletes/mutates one already-migrated row from `memory_artifacts` directly (simulating a process interrupted after the source read but before every row committed, or a caller retrying after a partial failure), reruns the same migration function, and asserts: (a) the deleted row is re-inserted exactly once, (b) every row the first run already committed successfully is not duplicated. Covers the recovery path `test_migration_is_idempotent` (T-61.22) does not: T-61.22 only proves a *fully clean* second run produces no duplicates — it never removes or perturbs any already-migrated row before rerunning, so an implementation that duplicates rows specifically on a partial/interrupted rerun (the scenario `CASMapTransaction.update()`'s real retry-with-backoff loop, `src/rush/memory/transactions.py:196-213`, exists to handle for its own store) would still pass T-61.22 while failing this test. |
| T-61.38 | `tests/test_phase61_store.py` | `test_recall_with_empty_allowlist_fails_closed` | A `recall()` call given an empty/absent session allowlist returns no rows and does not silently treat empty as "allow all". |
| T-61.39 | `tests/test_phase61_store.py` | `test_recall_scopes_to_session_allowlist` | Two records from different `source` sessions; a caller-supplied allowlist naming only one session's `source` returns only that session's rows. |

Coverage: 39 of 39 contract tests specified above are the complete set for this phase (T-61.01 through T-61.39 — the numbering runs past .36 after T-61.35/T-61.36 were added for the grounding-check and conflict-resolution gaps folded in from §3.2.2, T-61.37 was added for interrupted/partial-migration-rerun coverage per the migration-idempotency-key correction in §2.1/§9 P61.3, and T-61.38/T-61.39 were added for session-scoped read-allowlist coverage per §9 P61.9); §10's final gate re-runs all 39 by name, not a sample.
...
3. `src/rush/memory/migration.py` — one-shot migration functions, one per source (`migrate_preference_store`, `migrate_invariant_graph`, `migrate_checkpoint_journal`, `migrate_failure_ledger`, `migrate_patch_memory`, `migrate_flight_recorder`, `migrate_hook_signatures`, `migrate_session_memory`), each idempotent (T-61.22, T-61.37). `migrate_session_memory` reads existing `.rush/session_memory.json` (`session_memory.py:20` `DEFAULT_MEMORY_FILE`) via `SessionMemoryManager.load_records()`, writes each `SessionRecord` as one `MemoryArtifact` row (`subject="episodic"`, `family="experience"`, `origin_kind="session_memory"`, `origin_id=sha256(timestamp + tool_name + summary)` per record, since `SessionRecord` carries no id of its own), then renames `.rush/session_memory.json` to `.rush/session_memory.json.migrated` per Invariant 5 (§6.3) — the 9th source this phase absorbs (§3.1 goal 1).
...
8. **(Removed — no modified file here.)** rush's cross-tool memory-transport dispatcher (P61.10) is entirely new code (`src/rush/memory/transport.py`, already listed under New Files item 4); it does not modify `src/rush/continuity/providers.py`, whose `provider_command()` dict is session-resume CLI dispatch, a distinct concern this phase must not touch (§2.1 Drift 6).
...
- **Task ID:** P61.3.1
- **Binary Outcome:** `tests/test_phase61_migration.py` created with T-61.15 through T-61.22 and T-61.37; all 9 fail (RED).
- **Prerequisites:** P61.2.2.
- **Allowed Writes:** `tests/test_phase61_migration.py`.
- **Actions:**
  1. Author T-61.15 through T-61.22 and T-61.37 (§7), each seeding real fixture data in the OLD format (`CASMapTransaction` JSON files, `failure_ledgers` SQLite rows, checkpoint JSON, `PatchMemoryStore` SQLite rows, flight JSONL, hook signatures JSON) before asserting the migrated shape; T-61.37 additionally seeds one row directly into `memory_artifacts` post-first-run to simulate partial completion.
  2. Run `pytest tests/test_phase61_migration.py -v`, confirm all 9 RED.

#### P61.3.2 — GREEN: Implement Migration Functions & Compatibility Views
- **Task ID:** P61.3.2
- **Binary Outcome:** `src/rush/memory/migration.py` implements all seven migration functions (§8.1 New Files item 3); `preference_store.py`, `invariant_graph.py`, `merkle_invalidator.py`, `checkpoint_journal.py`, `failure_ledger.py`, `patch/memory.py`, `tools/flight_recorder.py`, `hook/tamper_detector.py` each become thin compatibility views (§8.1 Modified Files 1-4); all 9 of T-61.15-22 and T-61.37 pass; `src/rush/tools/continuity.py`'s restore path no longer short-circuits on the renamed physical checkpoint file (action 4 below), and `CheckpointJournal.save_checkpoint()`'s existing-path-return contract (`tests/test_phase41_memory_ship.py::test_checkpoint_journal`) still holds (action 5 below).
- **Prerequisites:** P61.3.1 RED.
- **Allowed Writes:** `src/rush/memory/migration.py`, `src/rush/memory/preference_store.py`, `src/rush/memory/invariant_graph.py`, `src/rush/memory/merkle_invalidator.py`, `src/rush/memory/checkpoint_journal.py`, `src/rush/memory/failure_ledger.py`, `src/rush/patch/memory.py`, `src/rush/tools/flight_recorder.py`, `src/rush/hook/tamper_detector.py`, `src/rush/tools/continuity.py` (its `_run_restore` does its own physical-file pre-check before ever calling `CheckpointJournal.restore_checkpoint()` — see action 4 below).
- **Actions:**
  1. Implement each migration function, idempotent: before inserting, `SELECT 1 FROM memory_artifacts WHERE origin_kind = ? AND origin_id = ?` and skip the insert if a row already exists (T-61.22). Derive `origin_id` per source since none of the 8 source formats carries a caller-facing unique id of its own: `checkpoint_journal` → the checkpoint `name` (already unique per `CheckpointJournal.save_checkpoint`); `failure_ledger` → its `fingerprint` (already `PRIMARY KEY`); `patch_memory` → its `error_signature` (already `PRIMARY KEY`); `preference_store`/`invariant_graph`/`merkle_invalidator` → their existing `CASMapTransaction` map key; `hook_signatures` → the hook file name key already used in `.rush/hook_signatures.json`; `flight_recorder` → `sha256(session_id + "|" + str(timestamp) + "|" + event_type)` computed from the JSONL row's own fields (flight events carry no native id — `flight_recorder.py:18-31` — so this is a derived, not a stored, key, stable across retries because it's a pure function of the row's existing fields, not of migration run order).
  ...
  3. Rename `.rush/sessions/flights/*.jsonl` (`FlightRecorder`) and `.rush/hook_signatures.json` (`HookTamperDetector`), and the preference/invariant/checkpoint satellite JSON files this task's own migration functions cover, with a `.migrated` suffix (never delete) per Invariant 5 (§6.3). Do **not** rename, move, or delete `.rush/cache.db` — `PatchMemoryStore`'s migration copies its `patch_memory` table rows into `TypedArtifactStore` and leaves the physical file in place and live, because `ResultCache` (`src/rush/cache.py:94-99`) shares that same file for an unrelated tool-result cache and is not part of this migration. **Also do not rename `.rush/session_memory.json` here** — it is migrated by P61.5.2's own `migrate_session_memory()` (§9 P61.5.2 action 2), a later task; if this task's rename swept it up too, P61.5.2 would find nothing at the original path when it runs. Record the exact renamed paths (JSONL/JSON only) in this task's evidence note within `docs/phase-plans/phase-61-implementation-evidence.md`.
  4. `.rush/sessions/<name>.json` (checkpoint's own satellite file) is renamed to `.migrated` by action 3 above, but `src/rush/tools/continuity.py`'s `_run_restore` (`continuity.py:330-347`) checks `session_dir.exists()` then `session_file.exists()` for that exact `.json` path *before* ever calling `CheckpointJournal(root).restore_checkpoint(name)` — after this task, that pre-check would always find the file absent (renamed to `.migrated`) and return `"skipped"` for every checkpoint, never reaching the now-working `CheckpointJournal`. Remove `_run_restore`'s own `session_dir`/`session_file` existence checks (`continuity.py:330-347`) and let `CheckpointJournal(root).restore_checkpoint(name)` be the sole existence authority — it already returns `None` for a missing checkpoint (existing behavior at `_run_restore`'s `if data is None:` branch, `continuity.py:349`), which `_run_restore` already handles.
  5. `tests/test_phase41_memory_ship.py::test_checkpoint_journal` (L32-35) asserts `journal.save_checkpoint(...)` returns a `Path` and `dest.exists()` is `True`. Preserve this: `checkpoint_journal.py`'s compatibility-view `save_checkpoint()` must still return a real, existing `Path` after this task — either by continuing to write the physical `.json` file as a durable secondary artifact in addition to the `TypedArtifactStore` row (not renamed to `.migrated`, since it is now current output, not legacy data), or by defining and documenting a new on-disk marker path `save_checkpoint()` creates and returns instead. Record the chosen approach in this task's evidence note; whichever is chosen, `dest.exists()` must hold immediately after `save_checkpoint()` returns.
  6. Run `pytest tests/test_phase61_migration.py -v`, then run the full existing suite for these 8 modules (`pytest tests/ -k "preference or invariant or merkle or checkpoint or failure_ledger or patch_memory or flight_recorder or tamper" -v`) to confirm zero regression in pre-existing tests for these modules, then run `pytest tests/test_phase41_memory_ship.py::test_checkpoint_journal tests/ -k continuity -v` to confirm the restore path and `save_checkpoint()`'s existing-path contract both still pass.
...
#### P61.4.2 — GREEN: Migrate `receipts.py` Off the Binary Quarantine Flag, `checkpoint_journal.py` Persists Through `TypedArtifactStore`
- **Task ID:** P61.4.2
- **Binary Outcome:** `continuity/receipts.py` still returns a plain `dict` from `save_receipt(project_root, handoff)` and reads only from the `checkpoint` dict passed into `restore_receipt(project_root, checkpoint)` — its two functions have no artifact-ID parameter and perform no direct store I/O (`receipts.py:62-107` confirms neither function does today, and neither gains one here); the `"authority": "historical_evidence", "state": "quarantined"` field is replaced with `trust_tier` sourced from `default_entry_tier` (§6.2). The actual `TypedArtifactStore` I/O for `family="handoff"` rows happens in `checkpoint_journal.py` (P61.3.2's compatibility view over the store), which calls `save_receipt()` for its dict content, embeds the result under the checkpoint's `metadata.handoff` key (unchanged shape), and writes/reads the whole checkpoint as one `MemoryArtifact` row — matching `restore_receipt`'s existing contract of reading `checkpoint.get("metadata", {}).get("handoff")` from whatever `checkpoint_journal.restore_checkpoint()` hands it. Both T-61.24-25 pass.
- **Prerequisites:** P61.4.1 RED.
- **Allowed Writes:** `src/rush/continuity/receipts.py`, `src/rush/memory/checkpoint_journal.py`.
- **Actions:**
  1. In `receipts.py`, replace the `"authority": "historical_evidence", "state": "quarantined"` emission with a `trust_tier` field sourced from `default_entry_tier` (§6.2). `save_receipt`/`restore_receipt` keep taking/returning plain dicts — no artifact ID is threaded through either signature.
  2. In `checkpoint_journal.py` (already becoming a `TypedArtifactStore` compatibility view per P61.3.2), write the checkpoint dict — receipt included — as one `MemoryArtifact` row with `family="handoff"`, `subject="active_context"`; `restore_checkpoint()` reads it back and hands the same dict shape to `restore_receipt()` unchanged, so `receipts.py`'s public signatures need no modification.
...
#### P61.5.2 — GREEN: Wire `session_memory.py` and `FlightRecorder` to the Unified Store, Migrate Legacy `session_memory.json`
- **Task ID:** P61.5.2
- **Binary Outcome:** New episodic writes from both `session_memory.py` and `FlightRecorder.record_event` land in `TypedArtifactStore` with `subject="episodic"`, `family="experience"`; `migration.py`'s `migrate_session_memory` (added by this task, §8.1 New Files item 3) moves every existing record already in `.rush/session_memory.json` into `TypedArtifactStore` before `session_memory.py`'s read path switches over, so no pre-existing record is silently orphaned; T-61.23 and T-61.37 (new: legacy-migration idempotency for `session_memory.json`, mirroring T-61.22's shape) both pass.
- **Prerequisites:** P61.5.1 RED.
- **Allowed Writes:** `src/rush/session_memory.py`, `src/rush/tools/flight_recorder.py`, `src/rush/memory/migration.py`.
- **Actions:**
  1. Confirm `log_subsystem`'s exact signature (recorded at P61.0.1) before modifying `session_memory.py`'s logging calls.
  2. Implement `migrate_session_memory()` in `migration.py`: read every existing `SessionRecord` via `SessionMemoryManager.load_records()` (`session_memory.py:41-51`), write each as a `MemoryArtifact` row (`subject="episodic"`, `family="experience"`, `origin_kind="session_memory"`, `origin_id=sha256(record.timestamp + record.tool_name + record.summary)`, idempotent per §6.1's origin-key correction), then rename `.rush/session_memory.json` to `.rush/session_memory.json.migrated` per Invariant 5 (§6.3).
  3. Wire both forward write paths (`session_memory.py`'s `record_turn`, `FlightRecorder.record_event`) through the same `sanitize_value`-then-`TypedArtifactStore.write()` sequence.
  4. Preserve `format_for_mcp()`'s `<rush_session_memory>` XML-framing contract (`session_memory.py:89-101`) by having it read from `TypedArtifactStore` instead of `load_records()`'s file read, same output shape.
  5. Run `pytest tests/test_phase61_episodic.py -v`, confirm GREEN; run `pytest tests/test_session_memory.py -v` to confirm zero regression in existing persistence/XML-framing tests.
...
- **Task ID:** P61.6.2
- **Binary Outcome:** `mine_mistakes()`'s per-revert findings are shaped (not written) as `subject="failure"`, `trust_tier="DERIVED"` candidate records; persisting them into `TypedArtifactStore` happens only through `MemoryTool.write()` (P61.12), never inline inside `mine_mistakes()` itself. **Correction: `mine_mistakes()` is called unconditionally, with no permission check, by `continuity/coordination.py`'s `recover_coordination()` (`src/rush/continuity/coordination.py:185`) — it runs on every `coordination_recovery` request, and `tools/continuity.py`'s dispatch for that operation never calls `check_permissions()` (contrast `_run_save`, `tools/continuity.py:233`, which gates on `_WRITE_PERMISSION` before any write). If `mine_mistakes()` itself called `TypedArtifactStore.write()`, every read-side recovery request would become an unpermissioned database write.** `mine_mistakes()` stays a pure git-log miner returning candidate dicts, exactly as it is today (§2.2: "a git-log miner, not a store") — no store call is added to it.
- **Prerequisites:** P61.6.1. (Not P61.12.2: that would create a cycle — P61.12.1's own Prerequisites is "P61.1.2 through P61.11.2," which includes this task. This task only shapes pure candidate dicts, per its Binary Outcome; it needs no `MemoryTool` to exist yet, only documents in its evidence note that persistence is the caller's responsibility via `MemoryTool.write()`, permissioned the same way `_run_save` gates `_WRITE_PERMISSION`.)
- **Allowed Writes:** `src/rush/memory/mistake_miner.py`.
- **Actions:**
  1. Add a pure shaping function in `mistake_miner.py` (no `TypedArtifactStore` import, no permission dependency) that maps `mine_mistakes()`'s per-revert findings to `subject="failure"`, `trust_tier="DERIVED"` candidate dicts.
  2. Document in this task's evidence note that persistence is the caller's responsibility via `MemoryTool.write()`, gated the same way `_run_save` gates `_WRITE_PERMISSION` (`tools/continuity.py:233`) — `continuity/coordination.py`'s unpermissioned `recover_coordination()` path must never call the persisting write.
  3. Verify no regression in `mine_mistakes()`'s existing git-log-parsing tests.
...
#### P61.8.2 — GREEN: Wire Skill Mining to `PluginTrustStore`/`AgentSkillGenerator`/`TypedArtifactStore`
- **Task ID:** P61.8.2
- **Binary Outcome:** A mined `subject="skill_pattern"` candidate is written to `TypedArtifactStore` (`trust_tier="DERIVED"`, `promoted_at=None`) so it exists as a queryable row for later admission/promotion sweeps; its generated SKILL.md has no working `rush plugin run` path until `PluginTrustStore.grant_trust()` is explicitly called for it; T-61.27 passes.
...
- **Actions:**
  ...
  2. Add a `TypedArtifactStore.write(subject="skill_pattern", trust_tier="DERIVED", content={...plugin identity, closure digest...}, source=..., ...)` call at the point a skill candidate is mined — this is the row Phase 62's `skill_admission_check` maintenance task (`docs/phase-plans/phase-62-memory-integration-layer-plan.md` §6.2) requires to exist; without it that sweep's `WHERE subject = 'skill_pattern'` query always returns zero rows.
  3. Add the trust-gate check: **Correction — `generate_skill_markdown(plugin: PluginSpec)` (`src/rush/plugins/skills_generator.py:8-30`) receives only a `PluginSpec`, which has no `digest` field and no `PluginTrustStore` instance in scope; `is_trusted()` is an instance method requiring both (`src/rush/plugins/trust_store.py:158-192`).** Inside `generate_skill_markdown()`, instantiate `PluginTrustStore()` (zero-arg — defaults to the user-owned ledger at `PluginTrustStore.DEFAULT_LEDGER_PATH`, `trust_store.py:65,67-72`, no new parameter needed) and resolve the digest from `plugin.closure.closure_digest` (`PluginClosureManifest.closure_digest: str`, `src/rush/plugins/closure.py:59`) when `plugin.closure is not None`. The generated markdown's `rush plugin run <name>` line is only emitted if `plugin.closure is not None and PluginTrustStore().is_trusted(plugin.name, plugin.closure.closure_digest)` returns true; when `plugin.closure is None` (no closure computed for this candidate), the line is omitted unconditionally — a mined skill candidate with no closure identity can never appear trusted.
  4. Run `pytest tests/test_phase61_skill_pattern.py -v`, confirm GREEN.

---

### P61.9 — Read-Side Scoping & Recall-Time Defense

**Correction: T-61.04 and T-61.05 do not cover this workstream.** T-61.04 (`test_recall_rescans_stated_signature_and_raises_on_mismatch`) tests signature-tamper detection; T-61.05 (`test_recall_scans_content_for_trojan_source_chars`) tests Trojan Source scanning. Neither exercises a session-scoped read allowlist. §1 Security Boundary item 5 and §3.1 Goal 3 both require the allowlist to "fail closed on empty input" (matching real precedent: `src/rush/release/provenance_policy.py:637-642` raises `UntrustedSignerError` when its allowlists are empty rather than allowing all) — but no invariant in §6.3, no test in §7, and no task card anywhere in this plan implements or verifies that behavior.

**Task ID:** P61.9.1 — RED: Define Session-Scoped Read Allowlist Contract Tests
- **Binary Outcome:** `tests/test_phase61_store.py` gains T-61.38 (`test_recall_with_empty_allowlist_fails_closed` — a `recall()` call given an empty/absent session allowlist returns no rows and does not silently treat empty as "allow all") and T-61.39 (`test_recall_scopes_to_session_allowlist` — two records from different `source` sessions; a caller-supplied allowlist naming only one session's `source` returns only that session's rows); both fail (RED).
- **Prerequisites:** P61.1.1.
- **Allowed Writes:** `tests/test_phase61_store.py`.

**Task ID:** P61.9.2 — GREEN: Implement Session-Scoped Read Allowlist in `recall()`
- **Binary Outcome:** `TypedArtifactStore.recall()` accepts a session allowlist parameter and fails closed on empty input per T-61.38-39.
- **Prerequisites:** P61.9.1 RED, P61.1.2.
- **Allowed Writes:** `src/rush/memory/store.py`.
- **Actions:** Implement the allowlist parameter on `recall()`, applied before Invariants 3/4/6 run on any returned row; empty/absent allowlist returns zero rows, never all rows. Run `pytest tests/test_phase61_store.py -v`, confirm T-61.38-39 GREEN.
...
- **Actions:**
  1. No grep needed — §2.1 Drift 6 confirms rush has no existing cross-tool memory-transport dict to migrate; `continuity/providers.py`'s `provider_command()` dict is session-resume CLI dispatch, a separate concern this task must not touch or reference as a migration source.
...
- **Allowed Writes:** `src/rush/memory/transport.py` only — no existing provider-dispatch file is modified (§2.1 Drift 6).
- **Actions:**
  ...
  7. **(Removed.)** `transport.py` is new, additive dispatch code with its own call sites (invoked wherever a memory artifact needs cross-tool handoff, per P61.4/P61.5/P61.6's family-tagged writes); it does not replace `provider_command()`'s call sites in `continuity/providers.py` or `tools/continuity.py`'s `provider_resume` operation, which remain session-resume CLI dispatch, untouched by this task (§2.1 Drift 6).
...
- **Allowed Writes:** `src/rush/tools/memory.py`, `src/rush/tools/__init__.py` (add `MemoryTool()` to `ALL_TOOLS`, alongside `SessionContinuityTool()`), `src/rush/catalog.py` (add a `"memory": ToolSpec(...)` entry to `TOOL_SPECS`, matching the `"continuity"` entry's shape at `catalog.py:115`, **and add a matching `"memory": "real_adapter"` entry to `_TOOL_MATURITY` at `catalog.py:744-797` — `catalog.py:798-799` runs `if set(_TOOL_MATURITY) != set(TOOL_SPECS): raise RuntimeError(...)` at import time, so a `TOOL_SPECS` entry with no `_TOOL_MATURITY` counterpart breaks every `catalog.py` import (CLI, MCP, and this phase's own tests) before any test runs**), `src/rush/cli.py` (add `"memory"` to the exclusion set at `cli.py:1201-1208`, and add a new `@cli.group(name="memory")` with `ask`/`write`/`promote`/`list`/`recall`/`maintain` subcommands, mirroring `@cli.group(name="session")` at `cli.py:1878` and its subcommands at `cli.py:1934-2009` — `maintain` is a real registered CLI leaf, its handler just returns `MemoryTool()`'s reserved `status="skipped"` result until Phase 62's P62.3.2 replaces the dispatch body), `src/rush/governance/public_operations.py` (add `explicit_pairs` entries — Action 6 below), `tests/test_phase57_public_operations.py` (update frozen manifest counts — Action 7 below).
- **Actions:**
  1. Implement `MemoryTool(ToolFn)`, `name = "memory"`, `__call__` accepting `operation: Literal["ask","write","promote","list","recall","maintain"]` plus operation-specific kwargs, mirroring `SessionContinuityTool`'s parameter style. `"maintain"` is reserved here (dispatched as `status="skipped", reason="not implemented until Phase 62"` until then) so Phase 62's maintenance sub-agent (§3.2.3 item 3) has a real CLI/MCP registration surface to wire into rather than inventing a new one — see that phase's own P62.3 task cards.
  ...
  4. Add a `"memory"` entry to `TOOL_SPECS` in `src/rush/catalog.py`, matching `"continuity"`'s fields (`catalog.py:115`), **and add `"memory": "real_adapter"` to the `_TOOL_MATURITY` dict (`catalog.py:744-797`) in the same edit — the module-level `if set(_TOOL_MATURITY) != set(TOOL_SPECS): raise RuntimeError(...)` check at `catalog.py:798-799` requires the two dicts' key sets to match exactly, and both edits must land together or `catalog.py` cannot be imported.**
  5. Add five `explicit_pairs` entries to `src/rush/governance/public_operations.py` (`public_operations.py:69`, dict literal), one per CLI leaf, all pointing at `mcp_tool="rush_memory"`: `"memory ask"` and `"memory list"` and `"memory recall"` → `effect_class="read-only"`, `kind_str="tool"`; `"memory write"` and `"memory promote"` → `effect_class="stateful-mutation"` (matching the `"release"` entry's pattern, `public_operations.py:77-83`), `kind_str="admin"`. `canonical_impl` for all five: `"rush.tools.memory:MemoryTool"`. `safe_probe`: `f"rush memory {subcommand} --help"` for each. **Correction: without this, `rush_memory` is misclassified read-only** — `build_operations_inventory()`'s step 3 (`public_operations.py:169-193`) auto-pairs a `TOOL_SPECS` entry only when its bare name is itself a Click leaf; putting `"memory"` in the CLI exclusion set (action 7 below) means no bare `memory` Click leaf ever exists — only `memory ask`/`memory write`/etc. — so step 3 cannot match, and `rush_memory` falls to step 5 (`public_operations.py:219-238`, unassigned MCP tools), which hardcodes `effect_class="read-only"` unconditionally for every operation including the mutating `write`/`promote` ones.
  6. Update `tests/test_phase57_public_operations.py`'s frozen counts (`total_operations`, `declared_transports` both/cli/mcp splits, `tool_count`/`admin_count`/`service_count` — currently 146/56/73/17/67/62/17 at lines 237-250) to reflect the five new operations this task adds.
  7. Add `"memory"` to the exclusion set at `cli.py:1201-1208`, then add `@cli.group(name="memory")` with `ask`/`write`/`promote`/`list`/`recall`/`maintain` subcommands, each constructing `MemoryTool()` and calling it with the matching `operation=` kwarg — following `@cli.group(name="session")` (`cli.py:1878`, subcommands at `1934-2009`) exactly. `maintain`'s subcommand is real and registered here; its call reaches `MemoryTool.__call__(operation="maintain", ...)`, which returns the reserved `status="skipped"` result (action 1) until Phase 62's P62.3.2 replaces that dispatch body — the CLI leaf itself is not new in Phase 62, only its behavior.
  8. Run `pytest tests/test_phase61_memory_tool.py tests/test_phase57_public_operations.py -v`, confirm all GREEN.
...
```bash
# 1. Run all 39 focused Phase 61 contract tests
...
# 3. Run complete test suite — record final count against P61.0.1's baseline, confirm baseline + 39
...
- [ ] `governance/remediation-phase-61.toml` created, documenting all 39 contract tests.
- [ ] Zero third-party dependencies introduced.
- [ ] Full pytest suite passes at baseline + 39 (exact numbers recorded in `docs/phase-plans/phase-61-implementation-evidence.md`).
