"""ast-grep adapter for Tree-sitter AST structural search and linting."""
...
class AstGrepEngine(Engine):
    ...
    def run(
        ...
        default_args = ["scan", "--json=compact"]
        ...
        return EngineResult(
            ...
            summary=f"ast-grep exit {proc.returncode}",
            duration_ms=0,
        )
    ...
    def normalize(self, raw: EngineResult, path: Path, tool_name: str) -> ToolResult:
            ...
            findings.append(
                {
                    ...
                    "rule": f"ast-grep/{item.get('ruleId', 'structural-pattern')}",
                    "severity": "fail" if severity == "error" else "warn",
                    "message": item.get("message", "AST structural match"),
                }
            )
        ...
        return ToolResult(
            ...
            summary=f"ast-grep: {len(findings)} structural finding(s)",
            findings=findings,
            raw=raw.get("parsed"),
        )
