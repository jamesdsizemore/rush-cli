...
def test_every_eligible_manifest_boundary_rejects_malformed_output() -> None:
    """T-58.21 (R-011 Governance Test): Probes all 146 operations from public-operations.toml with malformed payloads."""
    registry = get_operation_registry()
    assert isinstance(registry, OperationRegistry)
    assert len(registry._adapters) == 146
    ...
    assert tool_count == 67
    assert admin_count == 62
    assert service_count == 17
    assert tool_count + admin_count + service_count == 146
