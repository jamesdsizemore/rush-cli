...
| ISS-PROGRAM-GATES | P4–P5 required benchmark gate records are absent or inconsistent | Open | Critical — later coordination and provider work is forbidden until evidence is reconciled | P3 gate reconciliation | `BG-PRIV`, `BG-AUTH`, and `BG-CTX` are committed; historical handoff links are explicitly non-canonical. `BG-COORD` and route records remain absent. | Phase 4–5 schemas and approved provider routes | Program owner | Blocks P4–P5; does not block P3 | Reconcile committed decision records, exact gate IDs, and OmniRoute naming before each phase begins | BL-P4-00, BL-P5-00 | Gate-record audit before each phase T00 | No deferral decision recorded |
