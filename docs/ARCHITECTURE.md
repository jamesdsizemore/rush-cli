...
`provider_resume` remains inside the same continuity boundary. It projects only current goal/open work/freshness from a saved receipt into a fixed CLI profile for Claude Code, Codex CLI, or Antigravity, or a fixed loopback OmniRoute OpenAI-compatible request. Native executables receive typed argv; Windows batch launchers receive the projection through a per-process delayed-expansion environment variable so checkpoint-controlled text never enters CMD syntax. Network permission is required; Z.AI is deferred and 9Router remains unavailable pending its key/model contract. The route never reads a keychain, opens a browser, changes a provider profile, or returns the provider response.
