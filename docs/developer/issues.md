...
| ISS-PROGRAM-GATES | P5 enabled-route contract repair | Active | High — working user-owned routes must not be degraded to fixture/help-text claims | P5-T00 | Claude Code, Codex CLI, `agy`, and 9Router are installed; OmniRoute CLI and API are enabled. 9Router needs a gateway response in addition to CLI presence; OmniRoute uses its documented CLI/API forms. Z.AI is deferred. | Phase 5 provider routes and benchmark descriptors | Program owner | Does not block enabled-route implementation; blocks only unsafe/missing-profile execution | Preserve working routes; require bounded redacted result evidence and structured unavailable behavior | BL-P5-00 | Focused provider/router contract tests and explicit local probes | Pending P5 route-reconciliation commit |
