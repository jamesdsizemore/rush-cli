...
| ISS-PROGRAM-GATES | P5 route evidence is inconclusive or descriptor-inconsistent | Open | Critical — no live provider or router route may be claimed working without route-specific evidence | P5-T00 | Committed fixture routes exist; local B-D07–B-D10 records are inconclusive. 9Router help text is not a route result, OmniRoute descriptor/source identity is stale, and Z.AI profile detection is invalid. | Phase 5 provider routes and benchmark descriptors | Program owner | Blocks each live-route enablement; does not block capability/fixture repair | Reconcile exact command, profile boundary, endpoint/model response, source identity, and redaction per route | BL-P5-00 | Focused provider/router tests plus explicit local probe evidence | Pending P5 route-reconciliation commit |
