...
5. **Cryptographic Tamper Detection**: Verifies SHA-256 signatures of `.git/hooks/`. `HookTamperDetector` is a thin Phase 61 compatibility view over `TypedArtifactStore` (`.rush/memory.db`), the canonical store: if the live `.rush/hook_signatures.json` (regenerated by `rush hook install`) is present it's checked directly, otherwise the file has been migrated (renamed `.migrated`) and expected signatures are read from the store instead.
