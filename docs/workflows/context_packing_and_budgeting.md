...
## 1. Overview
`rush context pack --path src/rush/cli.py --json` returns a canonical continuity `ToolResult`: selected source evidence, estimated local tokens, omissions, and recovery state. `--path` is required. It skeletonizes the selected target file; it does not claim graph/PageRank ranking or provider-token savings. An insufficient budget returns `skipped` with an explicit omission reason.

## 2. Usage Examples
```bash
# Pack the CLI entrypoint with a 2000 token limit
rush context pack --path src/rush/cli.py --symbol cli --budget 2000
```

FastMCP invocation: `rush_context_pack(path="src/rush/cli.py", symbol="cli", budget=2000)`. Replace the Rush source path and symbol with the selected project's target.
