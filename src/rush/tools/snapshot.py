"""Snapshot testing tool supporting both local imported reports and execution."""
...
class SnapshotTool(ToolFn):
    """Import a local snapshot comparison or execute snapshot tests under --allow-slow."""

    name = "snapshot"

    @property
    def mcp_description(self) -> str:
        return "Import a local snapshot comparison or run snapshot tests under --allow-slow."
    ...
    def run(
        ...
        **options: object,
        ...
        if report_path is not None or path.is_file():
            ...
            try:
                ...
                return error_result(
                    self.name,
                    "snapshot-report",
                    f"refusing snapshot report outside target: {effective_report}",
            ...
            try:
                ...
                metrics = {
                    name: int(payload[name]) for name in ("matched", "mismatched")
                }
        ...
        # 2. Executed mode
        required_perms = (
            ExecutionPermissions(slow=True, artifact_write=True)
            if accept
            else ExecutionPermissions(slow=True)
        )
        ...
        if not is_satisfied:
            return skipped_result(
                self.name,
                None,
                f"snapshot: execution requires permission: {', '.join(missing_perms)}; never accepts baselines without --accept and --allow-artifact-write",
                duration_ms=elapsed_ms(start),
                metadata={
                    "execution": build_execution_metadata(
                        "executed",
        ...
        if not engine_on_path("pytest"):
            return skipped_result(
                self.name,
                "pytest-snapshot",
                "snapshot: snapshot test runner not available on PATH",
                duration_ms=elapsed_ms(start),
                metadata={
                    "execution": build_execution_metadata(
                        "executed",
        ...
        return ToolResult(
            tool=self.name,
            engine="pytest-snapshot",
            engine_version=None,
            status="ok" if proc.returncode == 0 else "warn",
            duration_ms=elapsed_ms(start),
            summary=f"snapshot: executed snapshot test runner (exit {proc.returncode})",
            findings=[],
            raw=proc.stdout,
            metadata={
                "evidence_source": "executed-runner",
                "baseline_mutated": accept,
                "execution": build_execution_metadata(
                    "executed",
                    requested=required_perms,
                    granted=permissions,
