...
def test_dependency_graph_topological_sort() -> None:
    p1 = WorkspacePackage(name="core", kind="python", root_path=Path("/core"), relative_path="packages/core")
    p2 = WorkspacePackage(name="cli", kind="python", root_path=Path("/cli"), relative_path="packages/cli", dependencies=("core",))
...
def test_affected_calculator_transitive(tmp_path: Path) -> None:
    p1 = WorkspacePackage(name="core", kind="python", root_path=tmp_path / "packages/core", relative_path="packages/core")
    p2 = WorkspacePackage(name="cli", kind="python", root_path=tmp_path / "packages/cli", relative_path="packages/cli", dependencies=("core",))
    p3 = WorkspacePackage(name="docs", kind="python", root_path=tmp_path / "packages/docs", relative_path="packages/docs")
...
def test_boundary_guard_illegal_relative_import(tmp_path: Path) -> None:
    ...
    bad_file.write_text('from "../../core/secret" import internal_val\n', encoding="utf-8")

    p = WorkspacePackage(name="cli", kind="python", root_path=pkg, relative_path="packages/cli")
