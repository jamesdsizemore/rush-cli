...
`NAME` must exactly match one of the 32 catalog tool names. Unknown names raise `RushConfigError`. `engine_args` and `check` are parsed for every tool, but the verified direct consumer is lint's engine arguments; `check` is not a universal policy switch. Prefer CLI `format --check` where supported.
