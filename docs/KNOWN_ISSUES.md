...
- **Data loss and disclosure:** `fix`, including `--dry-run`, can discard user changes; `ship clean` defaults to deletion; checkpoint/governance symlinks can escape the repository; sandbox fallback/cleanup is unsafe. Custom token-outline MCP output can expose secrets. Use disposable copies for affected runtime investigation. Status: planned — [P64-01–P64-05](phase-plans/phase-64-runtime-correctness-and-safe-execution-plan.md).
