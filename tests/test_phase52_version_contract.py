...
def test_development_fallback_is_explicit() -> None:
    """Verify that rush module handles PackageNotFoundError by explicitly falling back to '0.3.0'."""
    ...
    with patch(
        ...
        assert len(rush.__version__.split(".")) >= 3
...
def test_version_dev_fallback_on_package_not_found() -> None:
    """Retain alias for test_development_fallback_is_explicit for backwards compatibility."""
    test_development_fallback_is_explicit()


def test_every_runtime_consumer_uses_public_version(tmp_path: Path) -> None:
    """Verify that every runtime and generated consumer dynamically emits rush.__version__."""
    import json
    ...
    # 1. TypeScript generator header
    ts_out = TypeScriptContractGenerator.generate_interfaces(
        '{"components": {"schemas": {"Test": {"type": "object"}}}}'
    )
    assert f"// Auto-generated by Rush v{rush.__version__}." in ts_out

    # 2. Repo scaffolder
    scaffold_dir = tmp_path / "scaffold_test"
    scaffold_dir.mkdir()
    RepoScaffolder.init_repository(scaffold_dir)
    rush_toml = scaffold_dir / "rush.toml"
    assert rush_toml.exists()
    assert f'version = "{rush.__version__}"' in rush_toml.read_text(encoding="utf-8")

    # 3. SARIF report driver version
    sarif_doc = export_to_sarif([{"tool": "rush", "status": "passed", "findings": []}])
    driver = sarif_doc["runs"][0]["tool"]["driver"]
    assert driver["version"] == rush.__version__

    # 4. SARIF export tool driver semanticVersion
    sarif_export_doc = json.loads(SarifExporter.export_sarif([]))
    driver_export = sarif_export_doc["runs"][0]["tool"]["driver"]
    assert driver_export["semanticVersion"] == rush.__version__

    # 5. PR synthesizer template footer
    card = PrSynthesizer(PROJECT_ROOT).synthesize_pr_card()
    assert f"Rush CLI v{rush.__version__}" in card
