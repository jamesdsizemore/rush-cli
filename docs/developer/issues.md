...
| ISS-PROGRAM-GATES | P5 required benchmark route records are absent or inconsistent | Open | Critical — provider work is forbidden until route evidence is reconciled | P4 gate reconciliation | `BG-PRIV`, `BG-AUTH`, `BG-CTX`, and `BG-COORD` are committed; historical handoff links are non-canonical. Route records remain absent. | Phase 5 provider routes | Program owner | Blocks P5; does not block P4 | Reconcile committed route IDs and OmniRoute naming before Phase 5 begins | BL-P5-00 | Gate-record audit before Phase 5 T00 | No deferral decision recorded |
...
| ISS-P4-00 | Local ownership conflict was invisible to continuity handoff | Resolved | High — a resumed agent could not see another local owner before editing | P4-T01 | Focused conflict test holds an `agent-a` lock and verifies `agent-b` receives `skipped`/`conflict` without file mutation | Mesh lock and continuity result | Implementation agent | Non-blocking | Add read-only lock owner inspection and structured continuity coordination receipt | BL-P4-00 | `tests/test_phase49_trace_swarm_recorder.py` | Pending P4 commit |
