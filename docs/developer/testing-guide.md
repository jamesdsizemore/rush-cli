...
### Phase 59 Contract Tests (`tests/test_phase59_*.py`)
Covers unsigned provenance drafts, pinned statement structures, strict JSON parsing, signed DSSE verification, and deterministic engine conformance.

### Phase 61 Contract Tests (`tests/test_phase61_*.py`)
39 contract tests (T-61.01 through T-61.39) across `test_phase61_store.py`, `test_phase61_trust.py`, `test_phase61_migration.py`, `test_phase61_episodic.py`, `test_phase61_handoff.py`, `test_phase61_domain_knowledge.py`, `test_phase61_skill_pattern.py`, `test_phase61_transport.py`, `test_phase61_memory_tool.py`, and `test_phase61_adr.py`. Covers `TypedArtifactStore` WAL mode, redaction, signature re-verification, Trojan Source scanning, staleness, and FTS5 sync; the trust-tier write-promotion rule (screen, pre-filter, schema, grounding, corroboration) and conflict resolution; idempotent migration of all 8 satellite sources including interrupted-rerun recovery; episodic/handoff/domain-knowledge/skill-pattern subject wiring; the transport dispatcher's per-tool tier selection; `MemoryTool`'s dual CLI+MCP registration; and ADR-0049's supersession of ADR-0030. See `governance/remediation-phase-61.toml` for the full per-test manifest.

### Phase 62 Contract Tests (`tests/test_phase62_*.py`)
26 contract tests (T-62.01 through T-62.26) across `test_phase62_cache_gate.py`, `test_phase62_review_memory.py`, `test_phase62_maintenance.py`, `test_phase62_handoff_diff.py`, `test_phase62_attribution.py`, `test_phase62_api_staleness.py`, `test_phase62_expiry.py`, and `test_phase62_decision_schema.py`. Covers the token-savings cache gate's defended hit/miss and write-back-gating paths; review/dev citation of prior failure and architectural-decision records; the maintenance sub-agent's lease acquire/release, batch-size bound, and lease-expiry reclaim; cross-tool handoff delta diffs (including the direct-`run()`-caller regression); the AI-attribution trail (fix-commit-to-failure-record linkage via SHA, not full commit content); API-diff staleness (`ApiDiffer.diff_symbol()`, evaluated independently of merkle staleness, including the unavailable-`base_ref` `"unknown"` case); per-`trust_tier` expiry TTLs (`STATED` never, `DERIVED` 14d, `EXTERNAL_WRITE` 30d, `IMPORTED` 90d); and `DecisionRecordFields` round-trip through `mistake_miner.py`'s candidate-shaping function. See `governance/remediation-phase-62.toml` for the full per-test manifest.
