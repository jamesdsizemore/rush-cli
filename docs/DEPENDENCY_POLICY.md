...
3. **Automated Auditing**: Dependency auditing is a release check, not proof of zero vulnerabilities. Current CI ordering/tool failures remain open in [P64-20](phase-plans/phase-64-runtime-correctness-and-safe-execution-plan.md).

| Package | Purpose | Verification Contract |
|---|---|---|
| `click==8.4.2` | CLI command routing & option parsing | Tested via `tests/test_cli_registry.py` |
| `mcp==1.28.1` | FastMCP stdio server transport | Tested via `tests/test_mcp.py` |
