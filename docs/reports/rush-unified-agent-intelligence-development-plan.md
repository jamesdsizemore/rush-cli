...
Beginning goal: turn existing session/checkpoint/failure state into explicit redacted evidence with authority and stale-state behavior. Entry: P1 commit plus BG-AUTH and BG-PRIV decisions where persistence is committed. In scope: SessionMemoryManager, CheckpointJournal, FailureLedger, MerkleInvalidator, SecretRedactor adapters. Deferred: transcript import, semantic model, provider rendering. Allowed source: src/rush/session_memory.py, src/rush/memory/checkpoint_journal.py, failure_ledger.py, merkle_invalidator.py, invariant_graph.py, src/rush/safety/redactor.py, src/rush/tools/continuity.py; named tests below. Do-not-touch: provider adapters, dependencies, external accounts.
...
Checklist: [ ] P4-T00 discovery [ ] P4-WP1 ownership/staleness [ ] P4-WP2 replay/recovery [ ] P4-DOC audit [ ] P4-BI reconciliation [ ] P4-V checks [ ] P4-C commit.
...
Beginning goal: add only benchmark-approved, named provider routes under one permission/redaction/provenance contract. Entry: P4 commit; BG-PROV per route; BG-9R/BG-OMNI for named routers; BG-PROTO for bundle/renderer semantics. In scope: existing LLMProvider and existing OpenAI/Anthropic providers; only approved adapter seams. Deferred: unapproved provider, automatic routing, credential store, generic endpoint substitution, model selection.
...
P5-T02: for one route at a time, test the approved direct API or user-owned CLI profile contract using only the benchmark decision record; repeat for Codex, Claude Code, Antigravity, Z.AI, DeepSeek, OpenRouter, 9Router and OmniRouter only when their individual gate passes. Preconditions corresponding BG-PROV/BG-9R/BG-OMNI/BG-PROTO. Allowed files are exactly the benchmark-approved existing provider module or a new adapter path named in that approved decision, src/rush/tools/continuity.py, permissions.py, config.py, tests/test_providers.py and a benchmark-approved new focused test path. Do-not-touch any provider not approved by its gate, credential/keychain/home files, dependencies, router substitutions. Initial failure: no declared capability or unsafe request boundary. Implement bounded redacted adapter; refactor only route-local code. Negative: unavailable profile/timeouts/output limit returns skipped/error; Rush never signs in, opens browser or moves OAuth material. Docs: exact route, retention/privacy, fallback and configuration. Backlog one entry per route; issue one per blocked/failed gate, otherwise no issue. Verify approved contract tests/full/Ruff. Accept: route-specific pass record and production behavior tests are linked. Checklist P5-WP2/P5-T02.
...
|---|---|---|---|
| Benchmark authority and persistence decision | P2-T01 | ISS-BG-AUTH / BL-P2-01 | BG-AUTH decision record with replay/tombstone/instruction quarantine result. |
