...
`continuity` returns `tool`, `engine`, `engine_version`, `status`, `duration_ms`, `summary`, `findings`, `raw`, `artifacts`, and `metadata`. `metadata.operation` identifies `save`, `list`, or `restore`; `metadata.execution` records requested and granted permissions. A saved or restored receipt appears at `metadata.handoff` with `version`, `current_goal`, `open_work`, `historic_instruction`, `dependencies`, `freshness`, `failure_receipt`, `session_memory`, `redaction_count`, and `target_provider` (Phase 62: the receiving tool's `provider_id`, threaded through every `run()`-level caller of `SessionContinuityTool`, not only `__call__()`; the second-and-later `save` to the same `target_provider` sends a delta against `last_handoff_for_provider()`, not a full snapshot). `historic_instruction` and bounded historical session-memory records carry a `trust_tier` (`EXTERNAL_WRITE`/`DERIVED`/`IMPORTED` — never `STATED` on entry, Phase 61's unified typed-artifact schema, replacing the earlier binary `historical_evidence`/`quarantined` flag pair); receipt failures expose a fingerprint/redacted error, an explicit tombstone, or structured unavailable evidence—never a failed patch. Empty lists are `ok`; missing checkpoints and denied saves are `skipped`.
